...
if ~isempty(V1_TRANS.f.T1_param_subSubDir)
    ...
    wm_out_subDir = fullfile(V1_TRANS.f.T1_subDir, V1_TRANS.f.T1_param_subSubDir, tr_subDir_list{wm_taskID});
    ...
    wm_out_baseDir_param = fullfile(V1_TRANS.f.BaseDir, wm_out_subDir);
end
...
if ~isempty(V1_TRANS.f.T1_param2_subSubDir)
    ...
    wm_out_subDir = fullfile(V1_TRANS.f.T1_subDir, V1_TRANS.f.T1_param2_subSubDir, tr_subDir_list{wm_taskID});
    tmp = fullfile(V1_TRANS.f.BaseDir, wm_out_subDir);
    ...
    % Prep subfold
    mkdir(V1_TRANS.f.BaseDir, wm_out_subDir);
    wm_out_baseDir_param2 = fullfile(V1_TRANS.f.BaseDir, wm_out_subDir);
end
...
if ~isempty(V1_TRANS.f.T1_results_subSubDir)
    ...
    wm_out_subDir = fullfile(V1_TRANS.f.T1_subDir, V1_TRANS.f.T1_results_subSubDir, tr_subDir_list{wm_taskID});
    tmp = fullfile(V1_TRANS.f.BaseDir, wm_out_subDir);
    ...
    % Prep subfold
    mkdir(V1_TRANS.f.BaseDir, wm_out_subDir);
    wm_out_baseDir_results = fullfile(V1_TRANS.f.BaseDir, wm_out_subDir);
end
...
if ~isempty(V1_TRANS.f.T1_param_subSubDir)
    ...
    w.f.in_baseDir = fullfile(V1_TRANS.f.BaseDir, 'TrainTest');
    % w.f.in_baseDir = fullfile(V1_TRANS.f.BaseDir_for_trainTest, 'TrainTest');
    w.f.out_baseDir = wm_out_baseDir_param;

    % Copy "FBCSP_offline_trainTest_01 [xxx].mat" files from optimal FBCSP setup dir
    wm2a = 'FBCSP';

    if ~isfolder(fullfile(w.f.out_baseDir, wm2a))
        mkdir(fullfile(w.f.out_baseDir, wm2a))
    end

    wm1 = fullfile(w.f.in_baseDir, '- opt FBCSP');
    wm2 = fullfile(w.f.out_baseDir, wm2a);
    w_dirStruct = dir(wm1);
    ...
    if ~isdir(fullfile(w.f.out_baseDir, wm2a))
        mkdir(fullfile(w.f.out_baseDir, wm2a))
    end

    wm1 = fullfile(w.f.in_baseDir, wm2a, 'EEG');
    wm2 = fullfile(w.f.out_baseDir, wm2a);
    w_dirStruct = dir(wm1);
...
if ~isempty(V1_TRANS.f.T1_results_subSubDir)
    ...
    wm1 = fullfile(w.f.in_baseDir, '- Fig');
    wm2 = fullfile(w.f.out_baseDir, wm2a);
    w_dirStruct = dir(wm1);
...
%% Comments
% edited by Hardeep to make pltaform independent
% % Copy DA plots
% % _____________
