...
tmp = fullfile(V1_TRANS.f.BaseDir, V1_TRANS.f.DataSubDir);
...
% Prep Data subfold
wm_save_subDir = fullfile(V1_TRANS.f.DataSubDir, V1_TRANS.f.Subj1Sess1SubDir, V1_TRANS.f.classSubDir{1});
...
while tmp.wm < 100
    ...
    try
        ...
        fprintf('Trying to create %s\n', wm_save_subDir)
        fprintf('In BaseDir: %s\n', V1_TRANS.f.BaseDir)
        fprintf('Try number: %d\n', tmp.wm2)
        pause(10)
        tmp.wm = tmp.wm + 1;
    end
...
% Save EEG_rec.mat
save(fullfile(V1_TRANS.f.BaseDir, wm_save_subDir, 'EEG_rec.mat'), 'EEG_rec', '-v7.3');
clear EEG_rec

%% Comments
%% Made crossplatform compatible
% Edited by Hardeep on 2025-06-27
