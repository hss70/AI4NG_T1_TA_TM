...
if sudo docker ps -a | grep -q localstack; then
    echo "LocalStack container already exists, removing it..."
...
aws --endpoint-url=http://localhost:4566 s3 cp "s3://test-results-bucket/$USER_ID/$SESSION_ID/results/" test_results/ --recursive
...
if aws --endpoint-url=http://localhost:4566 s3 ls "s3://test-results-bucket/$USER_ID/$SESSION_ID/FBCSP_online_setup_prep_01 [online].json" &>/dev/null; then
  ...
  aws --endpoint-url=http://localhost:4566 s3 cp "s3://test-results-bucket/$USER_ID/$SESSION_ID/FBCSP_online_setup_prep_01 [online].json" test_results/
...
sudo docker network rm eeg-test-network || true

echo "Test complete! Results available in test_results/"